...
protocol LivePhotoKitProtocol {
    func extractResources(from livePhoto: PHLivePhoto, completion: @escaping (LiveImageURLs?) -> Void)
    func generate(from imageURL: URL?, videoURL: URL, progress: @escaping (CGFloat) -> Void, completion: @escaping (PHLivePhoto?, LiveImageURLs?) -> Void)
}

enum LivePhotoFileType: String {
    case jpg = "JPG"
    case mov = "MOV"
}

class LivePhotoKit: LivePhotoKitProtocol {
    // MARK: PUBLIC
    typealias LivePhotoResources = LiveImageURLs
    ...
    private func generateKeyPhoto(from videoURL: URL) -> URL? {
        ...
        guard let url = cacheDirectory?.appendingPathComponent(UUID().uuidString).appendingPathExtension(LivePhotoFileType.jpg.rawValue) else { return nil }
        do {
    ...
    public func generate(from imageURL: URL?, videoURL: URL, progress: @escaping (CGFloat) -> Void, completion: @escaping (PHLivePhoto?, LivePhotoResources?) -> Void) {
        ...
        guard let keyPhotoURL = _keyPhotoURL, let pairedImageURL = addAssetID(assetIdentifier, toImage: keyPhotoURL, saveTo: cacheDirectory.appendingPathComponent(assetIdentifier).appendingPathExtension(LivePhotoFileType.jpg.rawValue)) else {
            DispatchQueue.main.async {
                completion(nil, nil)
            }
            return
        }
        addAssetID(assetIdentifier, toVideo: videoURL, saveTo: cacheDirectory.appendingPathComponent(assetIdentifier).appendingPathExtension(LivePhotoFileType.mov.rawValue), progress: progress) { (_videoURL) in
            if let pairedVideoURL = _videoURL {
                _ = PHLivePhoto.request(withResourceFileURLs: [pairedVideoURL, pairedImageURL], placeholderImage: nil, targetSize: CGSize.zero, contentMode: PHImageContentMode.aspectFit, resultHandler: { (livePhoto: PHLivePhoto?, info: [AnyHashable : Any]) -> Void in
                    ...
                    DispatchQueue.main.async {
                        completion(livePhoto, .init(image: pairedImageURL, video: pairedVideoURL))
                    }
                })
            } else {
                DispatchQueue.main.async {
    ...
    private func extractResources(from livePhoto: PHLivePhoto, to directoryURL: URL, completion: @escaping (LivePhotoResources?) -> Void) {
        ...
        group.notify(queue: DispatchQueue.main) {
            ...
            completion(.init(image: pairedPhotoURL, video: pairedVideoURL))
        }
    }
    
    public func extractResources(from livePhoto: PHLivePhoto, completion: @escaping (LivePhotoResources?) -> Void) {
        if let cacheDirectory = cacheDirectory {
